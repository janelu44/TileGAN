...
def run_in_session(session, *args, **kwargs): # Run the specified ops in the specified session.
...
class Network:
    ...
    # Pickle import.
    def __updatestate__(self, state):
        # Parse imported module.
        module = imp.new_module('_tfutil_network_import_module_%d' % len(_network_import_modules))
        exec(self._build_module_src, module.__dict__)
        self._build_func = find_obj_in_module(module, self._build_func_name)
    ...
    # Create a clone of this network with its own copy of the variables.
    def clone_and_update(self, name=None, kwargs=None, func=None):
        build_func_name = None
        net = object.__new__(Network)
        net._init_fields()
        net.name = name if name is not None else self.name
        net.static_kwargs = kwargs if kwargs is not None else dict(self.static_kwargs)
        if func is not None:
            module_stage, build_func_name = import_module(func)
            module_src_stage = inspect.getsource(module_stage)
            net._build_module_src = module_src_stage
        else:
            net._build_module_src = self._build_module_src
        net._build_func_name = build_func_name if build_func_name is not None else self._build_func_name
        net._build_func = find_obj_in_module(module_stage, build_func_name) if build_func_name is not None else self._build_func
        net._init_graph()
        net.copy_vars_from(self)
        return net

    # update this network using the given kwargs.
    def update(self, kwargs=None):
        self.static_kwargs = kwargs if kwargs is not None else dict(self.static_kwargs)
        #self._init_graph()
        # Build template graph.
        with tf.variable_scope(self.scope, reuse=tf.AUTO_REUSE):
            assert tf.get_variable_scope().name == self.scope
    ...
    # update the size of the network graph using the given height and width.
    def update_latent_size(self, height, width):
        kwargs = dict(self.static_kwargs)
        depth = kwargs['latent_depth']
        kwargs['latent_size'] = [None, depth, height, width]
