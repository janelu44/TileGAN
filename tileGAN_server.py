...
class TFManager:
    ...
    def initNetworks(self):
        ...
        self.sessionA = tf.Session(graph=self.graphA)
        ...
        self.sessionB = tf.Session(graph=self.graphB)
        with self.sessionB.as_default():
            with open(self.networkPath, 'rb') as file:
                Gs = pickle.load(file)
            outRes = int(np.log2(Gs.output_shape[-1]))
            kwargs = {'in_res': inResB, 'out_res': outRes, 'latent_depth': depthB, 'latentSize': [None, depthB, inSizeB, inSizeB], 'label_size': 0,
        ...
        self.sessionC = tf.Session(graph=self.graphC)
        ...
        print(self.graphA, self.graphB, self.graphC)
        t2 = time.time()
    ...
    def getUpsampled(self, image):
        ...
        for i in range(self.height * self.width):  # ! currently picking random latent
            ...
            y = i // self.width
            x = i % self.width
            self.clusterGrid[y * self.latentSize:(y + 1) * self.latentSize, x * self.latentSize:(x + 1) * self.latentSize] = int(
                self.clusterLookup[index])
    ...
    def pasteLatents(self, sampleLatent, targetX, targetY, targetW, targetH, sourceX, sourceY, mode='identical'):
        ...
        sourceLatent = self.intermLatentGrid[:, :, sourceY - centerOffset:sourceY - centerOffset + latentSize,
                       sourceX - centerOffset:sourceX - centerOffset + latentSize]
        ...
        roi = 2 * latentSize
        outputImage = self.calculateOutputImage(self.intermLatentGrid, start=(max(targetY - roi, 0), max(targetX - roi, 0)),
                                                end=(min(targetY + targetH + 2 * roi, gridH), min(targetX + targetW + 2 * roi, gridW)),
                                                updateAll=False)
        t2 = time.time()
        ...
        if repeat:
            ...
            for x in np.arange(self.width):
                for y in np.arange(self.height):
                    self.clusterGrid[y * self.latentSize:(y + 1) * self.latentSize, x * self.latentSize:(x + 1) * self.latentSize] = clusters[
                        y * w + x]
    ...
    def calculateIntermediateLatents(self, latents):
        ...
        with self.graphA.as_default():
            with self.sessionA.as_default():
                ...
                intermediateLatents, _ = self.GsA.run(latents, in_res=2, out_res=self.mergeLevel, latent_depth=DEPTHS[0], minibatch_size=32,
                                                      num_gpus=1, out_dtype=np.float32)
    ...
    def getOutputFromLatents(self, latentList, updateUndoStack=True):
        ...
        for y in range(self.height):
            for x in range(self.width):
                self.intermLatentGrid[:, :, y * latentSize:(y + 1) * latentSize, x * latentSize:(x + 1) * latentSize] = intermLatents[
                                                                                                                        y * self.width + x, :,
                                                                                                                        pad:pad + latentSize,
                                                                                                                        pad:pad + latentSize]
    ...
    def perturbLatent(self, posX, posY, sourceX, sourceY, alpha, randomLatent=False, fromSamples=False, useCDF=True):
        ...
        sourceX = min(max(centerOffset, sourceX), gridW - (latentSize - centerOffset))  # constrain sourceX to grid dims
        sourceY = min(max(centerOffset, sourceY), gridH - (latentSize - centerOffset))  # constrain sourceY to grid dims
        intermLatent = self.intermLatentGrid[:, :, sourceY - centerOffset:sourceY - centerOffset + latentSize,
                       sourceX - centerOffset:sourceX - centerOffset + latentSize]
        ...
        self.intermLatentGrid[:, :, y_start:y_end, x_start:x_end] = (1 - alpha) * self.intermLatentGrid[:, :, y_start:y_end,
                                                                                  x_start:x_end] + alpha * intermLatent[:, :, t:latentSize - b,
                                                                                                           l:latentSize - r]

        roi = 2 * latentSize
        returnImage = self.calculateOutputImage(self.intermLatentGrid, start=(max(y_start - roi, 0), max(x_start - roi, 0)),
                                                end=(min(y_end + 2 * roi, gridH), min(x_end + 2 * roi, gridW)), updateAll=False)
        t3 = time.time()
    ...
    def calculateUnmergedOutputImage(self, intermLatentGrid, mergeSize=0):
        ...
        intermLatentsList = np.zeros((gridH * gridW, self.latentDepth, mergeSize,
                                      mergeSize))  # it's necessary to recalculate latentsList here because user may have dropped latent in overlapping region
        s = list(intermLatentsList.shape)
        for y in np.arange(gridH):
            for x in np.arange(gridW):
                intermLatentsList[y * gridW + x, :, :, :] = intermLatentGrid[:, :, y * mergeSize:(y + 1) * mergeSize,
                                                            x * mergeSize:(x + 1) * mergeSize]
        ...
        with self.graphC.as_default():
            with self.sessionC.as_default():
                ...
                _, outputs = self.GsC.run(intermLatentsList, in_res=self.mergeLevel + 1, out_res=self.outRes,
                                          latent_size=[None, s[1], mergeSize, mergeSize], latent_depth=s[1], minibatch_size=32, num_gpus=1,
        ...
        with self.graphB.as_default():
            with self.sessionB.as_default():
                ...
                with open(self.networkPath, 'rb') as file:
                    ...
                    channels = Gs.output_shape[1]
                outRes = int(np.log2(Gs.output_shape[-1]))
                kwargs = {'in_res': inResB, 'out_res': outRes, 'latent_depth': depthB, 'latentSize': [None, depthB, inSizeB, inSizeB],
                          'label_size': 0,
                          'num_channels': channels, 'resolution': size}
                self.GsB = Gs.clone_and_update("GsB", kwargs=kwargs, func='networks.G_new')
                ...
                if useChunks:  # process grid in chunks
                        ...
                        for x in range(startX, lastX + 1, chunkStride):
                            ...
                            try:
                                _, chunkOutput = self.GsB.run(latentChunk, in_res=self.mergeLevel + 1, out_res=self.outRes,
                                                              latent_size=[None, s[1], chunkShape, chunkShape], latent_depth=s[1], minibatch_size=32,
                                                              num_gpus=1, out_mul=127.5, out_add=127.5, out_dtype=np.uint8)
                    ...
                    _, self.output = self.GsB.run(intermLatentGrid, in_res=self.mergeLevel + 1, out_res=self.outRes,
                                                  latent_size=[None, s[1], s[2], s[3]], latent_depth=s[1], minibatch_size=32, num_gpus=1,
                                                  out_mul=127.5, out_add=127.5, out_dtype=np.uint8)
                    return np.squeeze(self.output)
    ...
    def deadLeaves(self, height, width):
        ...
        it = 0
        while np.any(
                occupancy == 0) or it > gridH * gridH * 10:  # second criterion is arbitrary emergency stopping condition for infinite loop - should not happen anymore anyway ;)
            it += 1
    ...
    def sampleFromCluster(self, latentCluster, numLatents=10, useCDF=True):
        ...
        with self.graphC.as_default():
            with self.sessionC.as_default():
                ...
                _, outputs = self.GsC.run(randIntermLatents, in_res=self.mergeLevel + 1, out_res=self.outRes,
                                          latent_size=[None, s[1], self.latentSize, self.latentSize], latent_depth=s[1], minibatch_size=32,
                                          num_gpus=1, out_mul=127.5, out_add=127.5, out_dtype=np.uint8)
